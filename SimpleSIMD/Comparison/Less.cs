    ...
    public static partial class SimdOps<T>
    {
        public static bool Less(in ReadOnlySpan<T> left, T right)
        {
            ...
            if (Vector.IsHardwareAccelerated)
            {
                ...
                for (; i < length; i++)
                {
                    if (Vector.LessThanAll(vrLeft.Offset(i), vRight) == false)
                    {
                        return false;
                    }
                }
            ...
            for (; i < left.Length; i++)
            {
                if (NumOps<T>.Less(rLeft.Offset(i), right) == false)
                {
                    return false;
                }
            }
        ...
        public static bool Less(in ReadOnlySpan<T> left, in ReadOnlySpan<T> right)
        {
            ...
            if (Vector.IsHardwareAccelerated)
            {
                ...
                for (; i < length; i++)
                {
                    if (Vector.LessThanAll(vrLeft.Offset(i), vrRight.Offset(i)) == false)
                    {
                        return false;
                    }
                }
            ...
            for (; i < left.Length; i++)
            {
                if (NumOps<T>.Less(rLeft.Offset(i), rRight.Offset(i)) == false)
                {
                    return false;
                }
            }
